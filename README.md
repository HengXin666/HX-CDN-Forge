...
- **📥 IDM 模式分段下载** — `reqByCDNRange()` 用 HTTP Range 将文件分给多节点并行下载，空闲节点动态劈半窃取慢节点任务
...
劈半窃取:
  Node C 保留 [5.9MB ── 6.7MB]  (与已下载部分连续)
...
特性：
- **自动探测** — HEAD 请求获取 Content-Length
- **等分初始段** — 按节点数平均分配
- **动态劈半** — 快节点完成后抢慢节点的未完成区间
- **连续性优化** — 劈半时原节点保留与已下载数据连续的部分
